...
public class AndroidInventoryScreen implements Screen {
	...
	private Stage stage;
	private Skin skin;
	public AndroidInventoryScreen(final Jayheart g, ExploreScreen e, ActorJ p){
		game = g;
	...
	public void show() {
		...
		for (float f=25; f<=camera.viewportHeight-25 && i>=0; f+=15){
			TextButton b = new TextButton(player.inventory().get(i).name(), textButtonStyle);
			b.addListener(new InventoryScreenButtonListener(game, ret, player, player.inventory().get(i)));
			b.setPosition(0, f);
			stage.addActor(b);
			i--;
		}
